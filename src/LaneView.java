...
public class LaneView implements LaneObserver, ActionListener {
	...
	public void receiveLaneEvent(LaneEvent le) {
		if (lane.isPartyAssigned()) {
			...
			if ((le.getFrameNum() == 1 && le.getBall() == 0 && le.getIndex() == 0) || le.getParty().getOld()){
				le.getParty().setOld(false);
				makeFrame(le);
			}
	...
	private void setScores(LaneEvent le) {
		...
		for (int k = 0; k < numBowlers; k++) {
			...
			for(int i=0; i<21; i++) {
				int value = ((int[]) le.getScore().get(bowlers.get(k)))[i];
				if(value != -1) {
					if(value == 10 && (i%2 == 0 || i == 19))
						ballLabel[k][i].setText("X");
					else if (i > 0  && i%2 == 1 && value + ((int[]) le.getScore().get(bowlers.get(k)))[i-1] == 10)
						ballLabel[k][i].setText("/");
					else if (value == -2)
						ballLabel[k][i].setText("F");
					else
						ballLabel[k][i].setText(Integer.valueOf(value).toString());
				}
			}
		}
	}
